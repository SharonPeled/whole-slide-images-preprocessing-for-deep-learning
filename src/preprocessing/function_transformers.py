...
def filter_non_tissue_tiles(slide, non_tissue_threshold, otsu_filter, black_filter, pen_filter):
    ...
    tile_black_fracs = filter_black_reduced_image(slide, **black_filter) if black_filter is not None else np.zeros(tile_background_fracs)
    tile_pen_fracs = filter_pen_reduced_image(slide, **pen_filter) if pen_filter is not None else np.zeros(tile_background_fracs)
